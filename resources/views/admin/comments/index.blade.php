...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-2">
        ...
        <span class="text-xs text-[#999]">All Comments</span>
    </div>
</div>
<div class="admin-card overflow-hidden">
    <table class="w-full text-sm">
        <thead class="admin-table-header">
            <tr><th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">Comment</th><th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">Article</th><th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">User</th><th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">Status</th><th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">Date</th><th class="text-right p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">Action</th></tr>
        </thead>
            ...
            <tr class="admin-hover-row">
                ...
                <td class="p-3">
                    <span class="text-xs px-2 py-0.5
                        ...
                        {{ $comment->status === 'approved' ? 'Approved' : ($comment->status === 'rejected' ? 'Rejected' : 'Pending') }}
                    </span>
                </td>
                <td class="p-3 text-[#999] text-xs">{{ $comment->created_at->diffForHumans() }}</td>
                <td class="p-3 text-right whitespace-nowrap">
                    @if($comment->status !== 'approved')
                    <form method="POST" action="{{ route('admin.comments.approve', $comment) }}" class="inline">@csrf <button type="submit" class="text-green-600 hover:text-green-800 text-xs mr-2">Approve</button></form>
                    @endif
                    @if($comment->status !== 'rejected')
                    <form method="POST" action="{{ route('admin.comments.reject', $comment) }}" class="inline">@csrf <button type="submit" class="text-yellow-600 hover:text-yellow-800 text-xs mr-2">Reject</button></form>
                    @endif
                    <form method="POST" action="{{ route('admin.comments.destroy', $comment) }}" class="inline" onsubmit="return confirm('Are you sure?')">@csrf @method('DELETE')<button type="submit" class="text-red-500 hover:text-red-700 text-xs">Delete</button></form>
                </td>
            </tr>
            @empty
            <tr><td colspan="6" class="p-6 text-center text-[#999]">No comments.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
