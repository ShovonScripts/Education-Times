...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-2">
        ...
        <h1 class="text-2xl font-bold">Newsletter সাবস্ক্রাইবার</h1>
    </div>
...
<form method="GET" class="flex gap-2 mb-4">
    <input type="text" name="search" value="{{ request('search') }}" placeholder="Email বা ফোন খুঁজুন..." class="admin-input flex-1">
    ...
    <table class="w-full text-sm">
        <thead class="admin-table-header">
            <tr>
                ...
                <th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">ফোন</th>
                <th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">চ্যানেল</th>
                <th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">Status</th>
                <th class="text-left p-3 font-semibold text-[#666] text-xs uppercase tracking-wider">সাবস্ক্রাইব</th>
        ...
        <tbody class="divide-y divide-[#e0e0e0] dark:divide-[#333]">
            ...
            <tr class="admin-hover-row">
                ...
                <td class="p-3">
                    <span class="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">{{ $subscriber->channel === 'email' ? 'Email' : 'হোয়াটসঅ্যাপ' }}</span>
                </td>
                <td class="p-3">
                ...
                <td class="p-3 text-right whitespace-nowrap">
                    <form method="POST" action="{{ route('admin.newsletter.toggle-active', $subscriber) }}" class="inline">@csrf
                        <button type="submit" class="text-xs {{ $subscriber->is_active ? 'text-yellow-600 hover:text-yellow-800' : 'text-green-600 hover:text-green-800' }} mr-2">{{ $subscriber->is_active ? 'Inactive করুন' : 'Active করুন' }}</button>
                    </form>
                    <form method="POST" action="{{ route('admin.newsletter.destroy', $subscriber) }}" class="inline" onsubmit="return confirm('নিশ্চিত?')">@csrf @method('DELETE')<button type="submit" class="text-red-500 hover:text-red-700 text-xs">ডিলিট</button></form>
                </td>
            </tr>
            @empty
            <tr><td colspan="6" class="p-6 text-center text-[#999]">কোনো সাবস্ক্রাইবার নেই।</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
