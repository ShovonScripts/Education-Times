...
<h1 class="font-serif text-2xl font-bold mb-6">নতুন Categories</h1>
<div class="bg-white dark:bg-[#1e1e1e] border border-[#e0e0e0] p-6 max-w-lg">
    <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-4">
        @csrf
        <div>
            <label class="block text-sm font-medium text-[#666] mb-1">Name (বাংলা) *</label>
        ...
        <div>
            <label class="block text-sm font-medium text-[#666] mb-1">প্যারেন্ট Categories (ফাঁকা রাখলে মূল Categories)</label>
            <select name="parent_id" class="w-full border border-[#e0e0e0] px-4 py-2.5 text-sm bg-white focus:outline-none focus:border-[#0d0d0d]">
                <option value="">— মূল Categories —</option>
